 ...
 */
    ...
    public function auth_update($twitter_id,$username,$token,$secret_token) {
        ...
        if (!(empty($user['User']))) {//既に登録してあった場合
            $this->save(array(
                'User' => array(
                    'id' => $user['User']['id'],
                    'username' => $username,
    ...
    public function update_total_words($user_id,$sum) {
        $user = $this->find('first',array('conditions' => array('id' => $user_id)));
        $user['User']['total_words'] = $sum;
